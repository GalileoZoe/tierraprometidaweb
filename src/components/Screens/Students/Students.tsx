...
import { FaAppleAlt, FaBeer, FaCannabis, FaFilePdf, FaFlask, FaFolder, FaPen, FaSmoking, FaSyringe, FaTrash } from 'react-icons/fa';
            ...
            {isLoading ? (
                ...
                <div style={{ maxHeight: '400px', overflowY: 'auto', border: '1px solid #ddd' }}>
                    <table style={{ borderCollapse: 'collapse', width: '100%' }}>
                        <thead>
                            <tr style={{ textAlign: 'center', position: 'sticky', top: 0, backgroundColor: '#f9f9f9', zIndex: 1 }}>
                                <th>PDF</th>
                                <th>Nombre</th>
                                <th>Apellidos</th>
                                <th>Usuario</th>
                            ...
                            {listStudents.length > 0 ? (
                                listStudents.map((student: Student) => (
                                    <tr key={student._id} style={{ textAlign: 'center', zIndex: 1, fontSize: '15px' }}>
                                        ...
                                        <td>{student.phone}</td>
                                        {drugIcons[student.drug as keyof typeof drugIcons] || <FaBeer className='icon' />}
